...
$sql = "SELECT `lich_su_nhap_kho`.`Ma_Lich_Su_Nhap_Kho`, `lich_su_nhap_kho`.`Thoi_Gian`, `mon_an`.`Ten_Mon`, `lich_su_nhap_kho`.`So_Luong_Cu`, `lich_su_nhap_kho`.`So_Luong_Moi`, `ho_so`.`Ho_Ten` FROM `lich_su_nhap_kho` LEFT JOIN `mon_an` ON `lich_su_nhap_kho`.`Ma_Mon` = `mon_an`.`Ma_Mon` LEFT JOIN `ho_so` ON `lich_su_nhap_kho`.`Nguoi_Nhap` = `ho_so`.`Ten_Dang_Nhap` WHERE `lich_su_nhap_kho`.`Thoi_Gian` >= '$thoiGianBatDau 00:00:00' AND `lich_su_nhap_kho`.`Thoi_Gian` <='$thoiGianKetThuc 23:59:59' ORDER BY `lich_su_nhap_kho`.`Thoi_Gian`;";
