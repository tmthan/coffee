...
$sql = "SELECT `ho_so`.`Ho_Ten` FROM `ho_so`, `hoa_don` WHERE `ho_so`.`Ten_Dang_Nhap` = `hoa_don`.`Thu_Ngan` AND `hoa_don`.`Ma_Hoa_Don` = $maHoaDon;";
$result = $conn->query($sql);
while ($row = $result->fetch_assoc()) {
	$thuNgan = $row['Ho_Ten'];
}
...
<html>
<head>
	<title>Hóa đơn tính tiền <?php echo($tenBan);?></title>
	<meta charset="utf-8">
...
while ($row = $result->fetch_assoc()){	
	...
	$sql4 = "SELECT DISTINCTROW `chi_tiet_hoa_don`.`Gia` FROM `chi_tiet_hoa_don` WHERE `chi_tiet_hoa_don`.`Ma_Mon` = $mamon AND `chi_tiet_hoa_don`.`Ma_Hoa_Don` = $maHoaDon;";
...
<tr style="border-top: 1px solid #333; margin-top: 5px; padding-top:  5px;">
	<td width="50%"> </td>		
	<td width="30%" colspan="2" class="text-right">Tổng cộng: </td>
...
<tr style="">
	<td width="80%" class="text-right" style="background: #fdfdfd;" colspan="3"><b>Thành tiền: </b></td>
...
<tr>
	...
	<td width="30%" class="text-right" colspan="2"> Khách trả:</td>
	<td width="20%" class="text-right"><?php echo(number_format($khachTra, 0));?>đ</td>
</tr>
<tr>
	<td width="50%"></td>
	<td width="30%" class="text-right" colspan="2"><b>Tiền thừa: </b></td>
	<td width="20%" class="text-right"><b><?php echo(number_format($tienThua, 0));?>đ</b></td>
</tr>
</table>
<br>
<div class="text-center">
