...
$giamGiaTatCa = locSo($_POST['giamgiatatca']);
...
while ($row = $result->fetch_assoc()) {
	...
	if ($trangThaiNau < 2 && $trangThaiNau >-1) {
...
if ($coTheThanhToan) {
	...
	$sql2 = "UPDATE `hoa_don` SET `Trang_Thai_Thanh_Toan` = 1, `Thoi_Gian_Ket_Thuc` = '$thoiGianKetThuc', `Tong_Tien` = $tongTien, `Phu_Thu`= $phuThu, `Ly_Do_Phu_Thu`= '$lyDoPhuThu', `Phu_Thu_Tat_Ca` = $phuThuTatCa, `Ly_Do_Phu_Thu_Tat_Ca` = '$lyDoPhuThuTatCa', `Giam_Gia`= $giamGia, `Giam_Gia_Tat_Ca` = $giamGiaTatCa, `Thanh_Tien`= $thanhTien,`Khach_Tra`= $khachTra,`Tien_Thua`= $tienThua,`Thu_Ngan`='$tenDangNhap' WHERE `hoa_don`.`Ma_Hoa_Don` = $maHoaDon;";	// cap nhat hoa don
	...
	echo "<div class='alert alert-success'>Đã thanh toán. Vui lòng đóng cửa sổ này và làm mới danh sách bàn để in hóa đơn</div>";
} else if ($tienThua < 0) {
